...
The `render.yaml` at the root of the repo defines the full infrastructure declaratively:
```yaml
services:
  - type: web
    name: washnet-pos
    ...
    envVars:
      ...
      - key: DATABASE_URL
        fromDatabase:
          name: washnet-db
          property: connectionString  # Auto-linked to the PostgreSQL instance
...
Every deploy on Render runs this script to prepare the environment:
```bash
#!/usr/bin/env bash
set -o errexit   # Fail fast on any error

pip install -r requirements.txt        # Install all dependencies
python manage.py collectstatic --no-input  # Bundle static assets (Whitenoise)
python manage.py migrate               # Apply DB migrations
python make_dev_users.py               # Seed default user accounts
python make_dev_items.py               # Seed default services & products
```
...
| `GEMINI_API_KEY` | Google Gemini AI API key | **Set manually in Render Dashboard** |

> ⚠️ `GEMINI_API_KEY` must be set manually — it is marked `sync: false` in `render.yaml` for security.
...
4. Set `GEMINI_API_KEY` manually in the Render dashboard.
